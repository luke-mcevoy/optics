...
Six pages, in dependency order; each is standard physics recomputed live, with the source stated and the numbers covered by tests.
...
- **Why error correction can work at all** — the repetition code and its p = ½ crossing; syndrome extraction on the three-qubit code showing the amplitudes untouched; a distance-3/5/7 surface code you can decode by hand (exact minimum-weight matching); a live Monte Carlo threshold plot with Λ read off against the paper's 2.14; erasure as bond percolation with its 50% threshold; and the stabilizer octahedron, Gottesman–Knill, Eastin–Knill and T-gate teleportation.
