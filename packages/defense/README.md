...
The guide explains a machine; it is not the place to derive hyperfine structure or the AC Stark shift. Those concepts live in a separate set of pages at [`#/foundations`](https://luke-mcevoy.github.io/neutral-atom-compute-visualizer/#/foundations), built from first principles in the same three levels with the same kind of live figures. Dotted links in the guide (`hyperfine↗`, `blockade↗`, `threshold↗`) jump to the relevant section. Nothing on those pages is from the paper — it is standard atomic and quantum physics with the source stated (Steck's *Rubidium 87 D Line Data*, CODATA), and every derived number is recomputed live and covered by tests.

Seven pages, in dependency order; each is standard physics recomputed live, with the source stated and the numbers covered by tests.

- **A qubit, physically** — the Bloch sphere with a measurement you can run; pure superposition versus a classical mixture (the same z statistics, different x statistics); resonant and detuned Rabi flopping; and a Ramsey sequence that turns a frequency into a fringe.
- **Inside the rubidium atom** — the level ladder zoomed from optical (384 THz) through fine (7.1 THz) and hyperfine (6.83 GHz) structure; a 3D vector model of how the nuclear and electron spins couple into F and m_F; the exact Breit–Rabi fan of the eight ground sublevels with the 575.15 Hz/G² clock-state curvature; and a dephasing picture that turns the 8.6 G field sensitivity into a memory time.
- **What light does to an atom** — the driven dipole and its polarizability (Re α for the trap, Im α for the scattering); an optical tweezer in 3D with depth, scattering rate and a trapped atom computed from the Grimm–Weidemüller–Ovchinnikov formulas; selection rules and optical pumping with Clebsch–Gordan weights; the Raman Λ scheme and why the paper detunes it to hundreds of GHz.
- **Cooling and seeing atoms** — the Maxwell–Boltzmann distribution from 300 K to 10 μK; Doppler cooling as a Langevin simulation with the Doppler and recoil limits; a 3D magneto-optical trap; and fluorescence imaging as a photon-budget and Poisson-discrimination problem with the heating it costs.
- **Rydberg atoms** — the Rydberg series with quantum defects on a log energy axis; scaling laws (radius n², spacing n⁻³, lifetime n³, C₆ n¹¹) on one log–log plot; two fluctuating dipoles and the van der Waals shift in 3D; blockade in the pair-state basis by integrating the Schrödinger equation, with the √2 Ω collective oscillation.
- **Entanglement and two-qubit gates** — correlation versus entanglement with a source you can switch between a Bell pair and a classical mixture; building a Bell pair one gate at a time with the reduced Bloch vectors shrinking; the CHSH test with sampled statistics; and what “fidelity” measures (populations plus a parity fringe).
- **Why error correction can work at all** — the repetition code and its p = ½ crossing; syndrome extraction on the three-qubit code showing the amplitudes untouched; a distance-3/5/7 surface code you can decode by hand (exact minimum-weight matching, union-find beyond 14 defects); a live Monte Carlo threshold plot with Pauli, erasure, and phenomenological (d-round, 2+1 D) modes, Λ read off against the paper's 2.14; erasure as bond percolation with its 50% threshold; and the stabilizer octahedron, Gottesman–Knill, Eastin–Knill and T-gate teleportation.
...
- **Physics tests.** `npm test` runs golden tests against closed-form results: Rayleigh limit and collection fraction for NA 0.65, Gaussian-beam z_R and w(z), blockade-radius scaling, the Rb 5s radius from the quantum defect and the ≈700× 53S/5s ratio, hydrogenic normalisation and node counts, and the two-atom blockade integrator (probability conservation; P_rr = sin⁴(Ωt/2) without interaction; √2 Ω collective oscillation and <1 % double excitation deep in blockade). The run program behind Fig. 8 is checked for phase tiling and for hardware constraints such as the two Rydberg colours always firing together. The Foundations modules are tested the same way — single-qubit kinematics (Rabi, Ramsey, purity), the Grimm–Weidemüller–Ovchinnikov trap formulas, Clebsch–Gordan weights and pumping into dark states, Doppler friction and the Doppler/recoil limits, Rydberg scaling laws, two-qubit gates and the CHSH bound (2 classically, 2√2 quantum), the surface-code decoder (corrects every weight-≤2 error at d = 5; below/above threshold ordering of d = 3 vs 5; union-find matching), the phenomenological d-round model (zero measurement error reduces to the 2D code; single data or measurement errors are corrected; d = 3 vs 5 ordering; threshold in the Wang–Harrington–Preskill window), and the erasure percolation model. The hyperfine module is checked against Steck: zero-field Breit–Rabi reproduces the F = 1, 2 energies, Landé g_F ≈ ±½, the 0.70 MHz/G and 1.40 MHz/G linear slopes, the 575.15 Hz/G² clock coefficient (and 42.5 kHz at 8.6 G), the traceless Zeeman sum over all eight sublevels at any field, and the Paschen–Back limit. The Raman π-pulse scattering identity Γ_sc·π/Ω_eff = πΓ/2Δ per beam (πΓ/Δ for both, Δ ≫ ω_hf) is checked against the two-photon formulas.
